...
class PathRatioComparator implements Comparator<Path> {
	...
	public int compare(Path path1, Path path2) {
		...
		double cost1 = Math.pow((double) path1.getTraversedCells().size(),
				POW_EXPONENT);
		double cost2 = Math.pow((double) path2.getTraversedCells().size(),
				POW_EXPONENT);

		double ratio1 = (double) path1.getScore() / cost1;
		double ratio2 = (double) path2.getScore() / cost2;
